...
const distortions = {
  ...
  xyDistortion: {
    ...
    getDistortion: `
      ...
      vec3 getDistortion(float progress){
        float movementProgressFix = 0.02;
        return vec3(cos(progress * PI * uFreq.x + uTime) * uAmp.x - cos(movementProgressFix * PI * uFreq.x + uTime) * uAmp.x, sin(progress * PI * uFreq.y + PI/2. + uTime) * uAmp.y - sin(movementProgressFix * PI * uFreq.y + PI/2. + uTime) * uAmp.y, 0.);
      }
    `,
    getJS: (progress, time) => {
      const movementProgressFix = 0.02;
  ...
  LongRaceDistortion: {
    ...
    getDistortion: `
      ...
      vec3 getDistortion(float progress){
        ...
        return vec3(sin(progress * PI * uFreq.x + uTime) * uAmp.x - sin(camProgress * PI * uFreq.x + uTime) * uAmp.x, sin(progress * PI * uFreq.y + uTime) * uAmp.y - sin(camProgress * PI * uFreq.y + uTime) * uAmp.y, 0.);
      }
    `,
    getJS: (progress, time) => {
...
class CarLights {
  ...
  init() {
    ...
    let colorArray = Array.isArray(this.colors) ? this.colors.map(c => new THREE.Color(c)) : [new THREE.Color(this.colors)];
...
class LightsSticks {
  ...
  init() {
    ...
    let colorArray = Array.isArray(options.colors.sticks) ? options.colors.sticks.map(c => new THREE.Color(c)) : [new THREE.Color(options.colors.sticks)];
...
const Hyperspeed = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    const options = { ...defaultOptions, ...effectOptions, colors: { ...defaultOptions.colors, ...effectOptions.colors } };
